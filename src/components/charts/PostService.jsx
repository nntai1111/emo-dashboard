// src/components/charts/PostCohortHeatmap.jsx
...
const PostCohortHeatmap = () => {
    const [cohortData, setCohortData] = useState([]);
    const [loading, setLoading] = useState(true);

    // Default: 12 tuần trước
        ...
        const fetchData = async () => {
            try {
                ...
                const res = await axios.get(
                    "https://api.emoease.vn/post-service/v1/posts/dashboard/cohorts",
                    {
                ...
                let transformed = series.map((item) => {
                    ...
                    item.points.forEach((p) => {
                        // store percent plus an absolute count (active) when available
                    ...
                    return {
                        cohortWeek: item.cohortWeek, // giữ lại để sort chính xác
                ...
                console.error("Lỗi tải post cohort:", err);
                alert("Không thể tải dữ liệu cohort bài viết");
            } finally {
                setLoading(false);
            }
        };
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
                <div>
                    <h2 className="text-sm font-semibold text-slate-900">
                        Tỷ lệ xem lại bài viết theo cohort tuần
                    </h2>
                    <p className="text-xs text-slate-500 mt-1">
                        % người dùng quay lại xem bài post đã đăng trong tuần đó (tính theo device)
                    </p>
                </div>
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-2">
                        <input
                            ...
                            className="px-2 py-1 border border-slate-300 rounded-lg text-xs focus:ring-2 focus:ring-green-500 outline-none"
                        />
                    </div>
                        ...
                        <thead className="text-sm">
                            <tr className="text-left text-slate-600 font-medium border-b">
                                <th className=" min-w-20 sticky left-0 bg-white z-10">
                                    ...
                                    >
                                        Tuần đăng bài {sortOrder === "newest" ? "↓" : "↑"}
                                    </button>
            ...
            {!loading && cohortData.length === 0 && (
                <div className="text-center py-12 text-slate-500 text-base">
                    Không có dữ liệu bài viết trong khoảng thời gian này
                </div>
            )}
        </div>
    );
...
export default PostCohortHeatmap;