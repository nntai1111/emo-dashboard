...
const SignUp = ({ referralCode }) => {
  ...
  useEffect(() => {
    ...
    checkExistingAuth();
  }, [navigate, isAuthenticated, dispatch, authCheckCompleted]);
  ...
  const handleAuthSuccess = async (data) => {
    try {
      ...
      // Create simple user object from form data
      const userData = {
        id: 1,
        email: form.email,
  ...
  const handleGoogleLogin = async (credentialResponse) => {
    ...
    try {
      ...
      // Attach referralCode when provided (from query param forwarded by route wrapper)
      if (referralCode) {
  ...
  if (isCheckingAuth) {
    return (
      <section className="relative overflow-hidden w-full min-h-[100dvh] py-10 md:py-16 flex items-center justify-center">
